...
class Example2(object):
  ...
  def __init__(self, article, article_extra, abstract_sentences, vocab):
    ...
    if config.pointer_gen:
      ...
      self.enc_input_extend_vocab_2, self.article_oovs_2 = data.article2ids(article_extra_words, vocab)

      # Get a verison of the reference summary where in-article OOVs are represented by their temporary article OOV id
      abs_ids_extend_vocab = data.abstract2ids(abstract_words, vocab, self.article_oovs+self.article_oovs_2)
  ...
  def pad_encoder_input_2(self, max_len, pad_id):
    while len(self.enc_input_2) < max_len:
      self.enc_input_2.append(pad_id)
    if config.pointer_gen:
      while len(self.enc_input_extend_vocab_2) < max_len:
...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list):
    ...
    for ex in example_list:
      ...
      ex.pad_encoder_input_2(max_enc_seq_len_2, self.pad_id)
    ...
    self.enc_batch_2 = np.zeros((self.batch_size, max_enc_seq_len_2), dtype=np.int32)
    self.enc_lens_2 = np.zeros((self.batch_size), dtype=np.int32)
    self.enc_padding_mask_2 = np.zeros((self.batch_size, max_enc_seq_len_2), dtype=np.float32)
    ...
    if config.pointer_gen:
      ...
      self.max_art_oovs_2 = max([len(ex.article_oovs_2) for ex in example_list])
      # Store the in-article OOVs themselves
      self.art_oovs = [ex.article_oovs for ex in example_list]
      self.art_oovs_2 = [ex.article_oovs_2 for ex in example_list]
      # Store the version of the enc_batch that uses the article OOV ids
      self.enc_batch_extend_vocab = np.zeros((self.batch_size, max_enc_seq_len), dtype=np.int32)
      self.enc_batch_extend_vocab_2 = np.zeros((self.batch_size, max_enc_seq_len_2), dtype=np.int32)
      for i, ex in enumerate(example_list):
...
class Batcher(object):
  ...
  def fill_example_queue(self):
    ...
    while True:
      try:
        (source1, source2, target) = input_gen.next() # read the next example from file. article and abstract are both strings.
	f.write(source1+"\t"+source2+"\t"+target+"\n")
      ...
      abstract_sentences = [sent.strip() for sent in data.abstract2sents(target)] # Use the <s> and </s> tags in abstract to get a list of sentences.
  ...
  def text_generator(self, example_generator):
    while True:
      ...
      try:
        article_text = e.features.feature['source1'].bytes_list.value[0] # the article text was saved under the key 'article' in the data files
        abstract_text = e.features.feature['target'].bytes_list.value[0] # the abstract text was saved under the key 'abstract' in the data files
        abstract_text_extra = e.features.feature['source2'].bytes_list.value[0]
      except ValueError:
      ...
      if len(article_text)==0: # See https://github.com/abisee/pointer-generator/issues/1
        ...
        yield (article_text, abstract_text_extra, abstract_text)
